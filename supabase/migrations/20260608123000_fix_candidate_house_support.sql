/*
  Defensive house-support migration.

  Why this exists:
  - Some environments still do not have `candidates.house`, which causes UI queries to fail.
  - Candidate visibility now supports `None`, which means "visible to all students".
  - Vote validation must allow candidates assigned to `None` while still locking each voter to one house.
*/
